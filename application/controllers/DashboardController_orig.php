...
//	    ini_set('max_execution_time', 15);
    ...
    public function getPartnerDashboardDetails($application) {
      ...
      Drunken_Header::setJs(array('https://maps.google.com/maps?file=api&amp;v=2.x&amp;key=AIzaSyDJ74cjZBM65nCGQ0AB1ZDiha_YpMZN_mw'));
      ...
      if(isset($params->vars['id'])) {
        $view->aq = Drunken_Acquisition::getAcquisitionById($params->vars['id']);
        ...
        if(strpos($_SERVER['HTTP_HOST'], 'dev') !== false) {
            $sBase = '../../crm-standortfabrik.de/public/user/';
        } else {
            $sBase = '../../standortfabrik/public/user/';
        }
        ...
        $sBaseLinkDir = strpos($_SERVER['HTTP_HOST'], 'dev') ? 'api.crm-standortfabrik-dev.de/data/' : 'api.crm-standortfabrik.de/data/';
                ...
                if($file != '.' && $file != '..') {
                    if(strpos($file, 'thumb_') === false) {
                        ...
                        if(is_file($sObiIsDir . '/' . 'thumb_' . $file)) {
                            $f['thumb'] = $protocol . $sBaseLinkDir . 'obi_is/' . $params->vars['id'] . '/' . $file;
                        }
