$(document).ready(function() {
	...
	if($('#search_first').length > 0) {
		...
		search_values_first['location_job_type_name'] = 'Standort Stellenart';
		...
		button.bind('click', function() {
			if(!empty_first) {
				...
				if($('#set-search-first').val() == 'status_first') {
				...
				} else if ($('#set-search-first').val() === 'location_job_type_name') {
					template += '<div class="fl" id="location_job_type_name_holder" style="margin-right:25px;">';
					template += '  <div class="remove button fl" style="width:150px; padding:4px 10px; margin-right:15px; margin-top:10px;">Remove</div>';
					template += '  <div class="fl" style="margin-top:10px;">';
					template += '    <select style="width:256px;" id="location_job_type_name" name="location_job_type_name">';
					jobTypes.forEach(jt => {
					...
					template.find('.remove').bind('click', function(e) {
						$('#'+$(e.currentTarget).parent().attr('id')).remove();
						search_values_selected_first.splice($.inArray('location_job_type_name', search_values_selected_first), 1);
						renewSelect();
					});

					template.insertBefore($('#search_first .last'));
					search_values_selected_first.push('location_job_type_name');
					renewSelect();
				} else {
					template += '<div class="fl" id="'+$('#set-search-first').val()+'_holder" style="margin-right:25px;">';
					template += '  <div class="remove button fl" style="width:150px; padding:4px 10px; margin-right:15px; margin-top:10px;">Remove</div>';
	...
	if($('#search_second').length > 0) {
		...
		search_values_second['location_address_second'] = 'Standort Adresse';
		search_values_second['location_job_type_name'] = 'Standort Stellenart';
		...
		button.bind('click', function() {
			if(!empty_second) {
				...
				if($('#set-search-second').val() == 'status_second') {
				...
				} else if ($('#set-search-second').val() === 'location_job_type_name') {
					template += '<div class="fl" id="location_job_type_name_holder" style="margin-right:25px;">';
					template += '  <div class="remove button fl" style="width:150px; padding:4px 10px; margin-right:15px; margin-top:10px;">Remove</div>';
					template += '  <div class="fl" style="margin-top:10px;">';
					template += '    <select style="width:256px;" id="location_job_type_name" name="location_job_type_name">';
					...
					template.find('.remove').bind('click', function(e) {
						$('#'+$(e.currentTarget).parent().attr('id')).remove();
						search_values_selected_second.splice($.inArray('location_job_type_name', search_values_selected_second), 1);
						renewSelect();
					});

					template.insertBefore($('#search_second .last'));
					search_values_selected_second.push('location_job_type_name');
					renewSelect();
				} else {
	...
	function getFirstAcquisitions() {
		...
		params.filter.acquisition_job_type_name = $('#location_job_type_name').val();
