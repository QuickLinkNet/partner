...
public static function getAcquisitionsNew($params) {
    ...
    $fields = array('acquisition_id' => 'aq.id as acquisition_id',
                    'acquisition_job_type_name' => '(SELECT name FROM job_types WHERE id = aq.job_types_id) as acquisition_job_type_name',
...
//    die($sql_big . $sql);
  ...
  public static function getAcquisitionById($aid) {
      $sql = 'SELECT aq.*, okz.community as city, states.name as state FROM acquisition as aq';
    ...
    if(file_exists($path . $aid)) {
      ...
      foreach($dirs as $key => $file) {
        if(strpos($file, 'original') !== false) {
          return 'https://www.crm-standortfabrik.de/user/acquisition/' . $aid . '/' . $file;
        }
      }
    }
    return false;
