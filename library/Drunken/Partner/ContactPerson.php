...
class Drunken_Partner_ContactPerson
{
  private $id = '';
  private $partner_id = '';
  private $salutation = '';
  ...
  public function addContactPerson() {
    $sql = 'INSERT INTO partner_contact_person (id, partner_id, salutation, first_name, last_name, phone, fax, email, username, password, last_login, last_activity, active) VALUES ';
    $sql .= ' ("", "'.$this->partner_id.'", "'.$this->salutation.'", "'.$this->first_name.'", "'.$this->last_name.'", "'.$this->phone.'", "'.$this->fax.'", "'.$this->email.'", "'.$this->username.'", "'.md5($this->password + 'standortfabrik').'", "", "", "'.$this->active.'")';
  ...
  public function setContactperson() {
     ...
     * Check E-Mail
     */
    $sql = 'SELECT * FROM partner_contact_person WHERE email = "'.$this->email.'" AND id != "'.$this->id.'"';
     ...
     * Check Username
     */
    $sql = 'SELECT * FROM partner_contact_person WHERE username = "'.$this->username.'" AND id != "'.$this->id.'"';
    $query = Drunken_Database::query($sql);
    if(mysqli_num_rows($query) > 0) {
    ...
    if(count($this->return['error_msg']) <= 0) {
      ...
      $sql = 'SELECT * FROM partner_contact_person WHERE id = "'.$this->id.'" LIMIT 1';
  	  $query = Drunken_Database::query($sql);
      ...
      $sql = 'UPDATE partner_contact_person SET salutation = "'.$this->salutation.'",
  ...
  public static function getContactPerson($id) {
    $sql = 'SELECT * FROM partner_contact_person';
  ...
  public static function checkUsername($username) {
    $sql = 'SELECT * FROM partner_contact_person WHERE username = "'.$username.'"';
    $query = Drunken_Database::query($sql);
  ...
  public static function checkEmail($email) {
    $sql = 'SELECT * FROM partner_contact_person WHERE email = "'.$email.'"';
    $query = Drunken_Database::query($sql);
  ...
  $query = Drunken_Database::query('SELECT * FROM partner_contact_person WHERE username = "'.$email_username.'" OR email = "'.$email_username.'"');
		if(mysqli_num_rows($query) > '0') { return true; }
  }
  
  public static function setPartnerContactPersonActive($pid, $active) {
	  $sql = 'UPDATE partner_contact_person SET active = ' . $active . ' WHERE id = ' . $pid;
	  if(Drunken_Database::query($sql)) {
	    
	    $partner = Drunken_Partner_ContactPerson::getContactPersonsById($pid);
	    
      $mailer = new Drunken_Mailer();
  		$mailer->subject = 'Reaktivierung Ihres Accounts';
  		$mailer->from_name = 'Standortfabrik';
  		$mailer->addresses = array($partner->email);
  		$mailer->message = 'Hallo '.$partner->first_name.' '.$partner->last_name.', <br /><br />';
  ...
  public static function setPartnerContactPersonInactive($pid, $active) {
	  $sql = 'UPDATE partner_contact_person SET active = ' . $active . ' WHERE id = ' . $pid;
	...
	public static function getContactPersonsByPartnerId($id) {
	  $sql = 'SELECT * FROM partner_contact_person WHERE partner_id = "'.$id.'"';
	  $query = Drunken_Database::query($sql);
	  
	  $partner_contacts = array();
	  
	  while($row = Drunken_Database::fetchObject($query)) {
	    array_push($partner_contacts, $row);
	  }
	...
	public function getContactPersonById($id) {
	  return Drunken_Database::fetchObject(Drunken_Database::query('SELECT * FROM partner_contact_person WHERE id = "'.$id.'" LIMIT 1'));
	}
}
?>